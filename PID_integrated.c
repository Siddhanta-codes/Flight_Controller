...
volatile uint32_t pidCounter = 0; // Counter to track 500ms intervals
...
void stabilize_platform_y() {
    ...
    float setpoint = 0.0; // Ideal y_accel value
    float error = setpoint - y_accel;

    integral += error * 0.5; // 500ms interval
    float derivative = (error - previous_error) / 0.5;

    float output = Kp * error + Ki * integral + Kd * derivative;

    // Save error for the next loop iteration
    previous_error = error;

    if (output < -30) { // Right tilt
        if (TIM5->CCR1 < 200 && TIM3->CCR1 > 100) {
            TIM3->CCR1 -= 1;
            TIM5->CCR1 += 1;
        } else if (TIM3->CCR1 < 100) { // CCR2 = 200
...
//		MPU6050_Read_Accel ();
		if (pidCounter >= 500) {
		   ...
		   pidCounter = 0; // Reset counter after running PID control
		}
		//stabilize_platform_y();
